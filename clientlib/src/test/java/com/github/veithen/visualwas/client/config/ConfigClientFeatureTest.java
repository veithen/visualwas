...
public class ConfigClientFeatureTest {
    ...
    public void test() throws Exception {
        ...
        transport.addExchanges(ConfigClientFeatureTest.class, "getServerMBean", "queryNames", "invoke-resolve", "invoke-getAttribute", "invoke-getAttributes", "invoke-discard");
        ...
        ObjectName threadPool = config.resolve(session, "Server=server1:ThreadPoolManager=:ThreadPool=WebContainer")[0];
        assertEquals(Integer.valueOf(60000), config.getAttribute(session, threadPool, "inactivityTimeout"));
        AttributeList attrs = config.getAttributes(session, threadPool, new String[] { "minimumSize", "maximumSize" }, false );
        assertEquals(5, attrs.size());
        assertAttribute(attrs, 0, "minimumSize", Integer.valueOf(5));
        assertAttribute(attrs, 1, "maximumSize", Integer.valueOf(10));
        // TODO: use constants here
        assertAttribute(attrs, 2, "_Websphere_Config_Data_Id", new ConfigDataId("cells/phobosNode03Cell/nodes/phobosNode03/servers/server1", "server.xml#ThreadPool_1183121908657"));
