...
public class AdminServiceAnnotationProcessor implements AnnotationProcessor {
    ...
    public void processOperation(OperationBuilder operationBuilder) {
        ...
        String operationName =
                operationAnnotation != null && !operationAnnotation.name().isEmpty()
                        ? operationAnnotation.name()
                        : operationBuilder.getName();
        ...
        if (!hasIOException) {
            throw new InterfaceFactoryException(
                    "Operation " + operationName + " doesn't declare IOException");
        }
        ...
        for (int i = 0; i < paramCount; i++) {
            ...
            if (paramAnnotation == null) {
                throw new InterfaceFactoryException(
                        "Missing @Param annotation for operation " + operationName);
            }
        ...
        operationBuilder.addAdapter(
                ...
                new OperationHandler(
                        ...
                        operationAnnotation != null && operationAnnotation.suppressHeader()));
    }
