...
final class MarshallingHandler implements Handler<Invocation, Object> {
    ...
    public CompletableFuture<Object> invoke(InvocationContext context, Invocation invocation) {
        ...
        OperationHandler operationHandler =
                invocation.getOperation().getAdapter(OperationHandler.class);
                            ...
                            try {
                                ...
                                if (response.isFault()) {
                                    try {
                                        exception =
                                                (Throwable)
                                                        faultReasonHandler.extractValue(
                                                                responseBody.getFault().getReason(),
                                                                contextImpl);
                                    } catch (TypeHandlerException ex) {
                                        exception =
                                                new ConnectorException(
                                                        "The operation has thrown an exception, but it could not be deserialized",
                                                        ex);
                                    }
                                } else {
                                    try {
                                        result =
                                                operationHandler.processResponse(
                                                        responseBody.getFirstElement(),
                                                        contextImpl);
