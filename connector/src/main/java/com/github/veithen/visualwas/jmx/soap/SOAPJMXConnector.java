...
public class SOAPJMXConnector implements JMXConnector {
    ...
    /**
     * Name of the attribute that specifies the connect timeout. The value must be an {@link
     * Integer} representing the timeout in milliseconds.
     */
     ...
     * attribute must be an instance of {@link ClassLoaderProvider}. If it is specified, then {@link
     * JMXConnectorFactory#DEFAULT_CLASS_LOADER} will be ignored. If neither {@link
     * #CLASS_LOADER_PROVIDER} nor {@link JMXConnectorFactory#DEFAULT_CLASS_LOADER} is specified,
     * then the connector will use the thread context class loader.
     */
    ...
    private synchronized void internalConnect(Map<String, ?> env) throws IOException {
        ...
        if (jmxCredentials != null) {
            attributes.set(
                    Credentials.class,
                    new BasicAuthCredentials(jmxCredentials[0], jmxCredentials[1]));
        }
        ...
        ClassLoaderProvider classLoaderProvider =
                (ClassLoaderProvider) env.get(CLASS_LOADER_PROVIDER);
        if (classLoaderProvider == null) {
            ClassLoader cl = (ClassLoader) env.get(JMXConnectorFactory.DEFAULT_CLASS_LOADER);
            classLoaderProvider =
                    cl == null ? ClassLoaderProvider.TCCL : new SimpleClassLoaderProvider(cl);
        }
        ...
        connectorConfigBuilder.addFeatures(
                new ConnectionIdFeature(connectionId), NotificationDispatcherFeature.INSTANCE);
        ...
        try {
            // TODO: we should call isAlive here and save the session ID (so that we can detect
            // server restarts)
            connector.getServerMBean();
        } catch (IOException ex) {
            connectionBroadcaster.sendNotification(
