...
final class SecurityInterceptor implements Interceptor<SOAPEnvelope, SOAPResponse> {
    ...
    @Override
    public CompletableFuture<? extends SOAPResponse> invoke(
            InvocationContext context,
            SOAPEnvelope request,
            Handler<SOAPEnvelope, SOAPResponse> nextHandler) {
        ...
        if (credentials != null) {
            ...
            if (credentials instanceof BasicAuthCredentials) {
                ...
                factory.createSOAPHeaderBlock("username", null, header)
                        .setText(basicAuthCreds.getUsername());
                factory.createSOAPHeaderBlock("password", null, header)
                        .setText(basicAuthCreds.getPassword());
