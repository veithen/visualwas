...
public abstract class AbstractObjectNamePatternMBeanLocator implements MBeanLocator {
    private CompletableFuture<ObjectName> queryRequiredSingleton(
            AdminService adminService, ObjectName pattern) {
        return adminService
                .queryNamesAsync(pattern, null)
                .thenApply(
                        names -> {
                                ...
                                if (it.hasNext()) {
                                    throw new CompletionException(
                                            new InstanceNotFoundException(
                                                    "Found multiple MBeans matching " + pattern));
                                } else {
                                    return mbean;
                                }
                            } else {
                                throw new CompletionException(
                                        new InstanceNotFoundException(pattern + " not found"));
                            }
                        });
    }
    ...
    public final CompletableFuture<ObjectName> locateMBean(AdminService adminService) {
        return producePattern(adminService)
                .thenCompose(pattern -> queryRequiredSingleton(adminService, pattern));
    }
