...
final class NotificationDispatcherImpl implements NotificationDispatcher, CloseListener {
    ...
    // We use a single subscription for all listeners (to avoid blocking multiple threads on the
    // server side)
    ...
    public synchronized void removeNotificationListener(
            ...
            NotificationFilter filter,
            Object handback)
            throws IOException, ListenerNotFoundException {
        for (Iterator<NotificationListenerRegistration> it = registrations.iterator();
                it.hasNext(); ) {
    ...
    private void run() {
        ...
        while (true) {
            ...
            try {
                ...
                synchronized (this) {
                    // If the handle changed, then somebody else has reregistered the subscription
                    // already
                    if (subscriptionHandle == this.subscriptionHandle) {
                        try {
                            this.subscriptionHandle =
                                    service.addSubscription(subscriptionInfo, null);
                ...
                log.warn(
                        "Failed to retrieve notifications for subscription " + subscriptionHandle,
                        ex);
                doDelay = true;
            }
        }
