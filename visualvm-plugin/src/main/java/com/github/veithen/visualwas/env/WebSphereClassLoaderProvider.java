...
public final class WebSphereClassLoaderProvider implements ClassLoaderProvider, PreferenceChangeListener {
    ...
    public synchronized ClassLoader getClassLoader() {
        if (classLoader == null) {
            ...
            if (wasHome != null) {
                try {
                    classLoader = new WebSphereRuntimeClassLoader(new File(wasHome), WebSphereClassLoaderProvider.class.getClassLoader());
                    if (log.isLoggable(Level.INFO)) {
                        log.info("Successfully created class loader for WebSphere runtime at " + wasHome);
                    }
                } catch (IOException ex) {
                    // Normally, the option panel does the necessary verifications, so that we should rarely get here
                    log.log(Level.SEVERE, "Unable to create the class loader for the WebSphere runtime", ex);
                }
            }
            if (classLoader == null) {
