...
public class SignerExchangeDialog extends JDialog {
    ...
    public SignerExchangeDialog(Window window, X509Certificate[] chain) {
        ...
        Mnemonics.setLocalizedText(
                label, NbBundle.getMessage(SignerExchangeDialog.class, "LBL_untrusted_warning"));
        add(
                label,
                new GridBagConstraints(
                        0, 0, 1, 1, 0.0, 0.0, WEST, NONE, new Insets(2, 0, 2, 0), 0, 0));
        final JList<X509Certificate> certSelector =
                new JList<>(new CertificateChainListModel(chain));
        ...
        add(
                certSelector,
                new GridBagConstraints(
                        0, 1, 1, 1, 1.0, 0.0, WEST, HORIZONTAL, new Insets(2, 0, 2, 0), 0, 0));

        final X509CertificatePanel certPanel = new X509CertificatePanel();
        add(
                certPanel,
                new GridBagConstraints(
                        0, 2, 1, 1, 1.0, 0.0, WEST, HORIZONTAL, new Insets(2, 0, 2, 0), 0, 0));
        certSelector.addListSelectionListener(
                        ...
                        new AbstractAction() {
                            ...
                            public void actionPerformed(ActionEvent e) {
                                TrustStore.getInstance()
                                        .addCertificate(certSelector.getSelectedValue());
                                dispose();
        ...
        Mnemonics.setLocalizedText(
                addButton,
                NbBundle.getMessage(SignerExchangeDialog.class, "LBL_Add_to_trust_store"));
        ...
        Mnemonics.setLocalizedText(
                cancelButton, NbBundle.getMessage(SignerExchangeDialog.class, "LBL_Cancel"));
        buttonPanel.add(cancelButton);
        add(
                buttonPanel,
                new GridBagConstraints(
                        0, 3, 1, 1, 0.0, 0.0, WEST, HORIZONTAL, new Insets(2, 0, 2, 0), 0, 0));
     ...
     *
     * @return <code>true</code> if the user has added a certificate to the trust store, <code>false
     *     </code> if the user has cancelled or closed the dialog
     */
